import React, { useEffect, useState } from 'react';
import api from '../services/api';
import { useWebSocket } from '../context/WebSocketContext';
import { 
  Cpu, 
  AlertTriangle, 
  Activity, 
  Zap, 
  CheckCircle,
  Clock,
  Compass,
  Download
} from 'lucide-react';
import {
  ResponsiveContainer,
  LineChart,
  Line,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  Legend
} from 'recharts';

interface Stats {
  total_equipments: number;
  active_alerts: number;
  running_equipments: number;
  availability_rate: number;
  total_consumption: number;
}

interface Equipment {
  id: number;
  name: string;
  type: string;
  location: string;
  status: string;
}

interface ChartPoint {
  time: string;
  temperature: number;
  vibration: number;
  pressure: number;
  voltage: number;
  consumption: number;
}

const DashboardPage: React.FC = () => {
  const { subscribe } = useWebSocket();
  const [stats, setStats] = useState<Stats>({
    total_equipments: 0,
    active_alerts: 0,
    running_equipments: 0,
    availability_rate: 0,
    total_consumption: 0
  });
  
  const [equipments, setEquipments] = useState<Equipment[]>([]);
  const [selectedEquipId, setSelectedEquipId] = useState<number | null>(null);
  const [liveData, setLiveData] = useState<Record<number, ChartPoint[]>>({});
  const [loading, setLoading] = useState(true);

  // Fetch initial stats and list of equipments
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const statsRes = await api.get('/dashboard/stats');
        setStats(statsRes.data);

        const equipRes = await api.get('/equipments');
        setEquipments(equipRes.data);
        if (equipRes.data.length > 0) {
          const firstEquipId = equipRes.data[0].id;
          setSelectedEquipId(firstEquipId);
          
          // Fetch last 15 measurements for the selected equipment to prepopulate chart
          try {
            const historyRes = await api.get(`/equipments/${firstEquipId}/measurements?limit=15`);
            const formattedHistory: ChartPoint[] = historyRes.data.reverse().map((m: any) => ({
              time: new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
              temperature: parseFloat(m.temperature),
              vibration: parseFloat(m.vibration),
              pressure: parseFloat(m.pressure),
              voltage: parseFloat(m.voltage),
              consumption: parseFloat(m.consumption)
            }));
            setLiveData(prev => ({ ...prev, [firstEquipId]: formattedHistory }));
          } catch (err) {
            console.error("Failed to fetch equipment history:", err);
          }
        }
      } catch (error) {
        console.error("Error loading dashboard data:", error);
      } finally {
        setLoading(false);
      }
    };

    fetchInitialData();
  }, []);

  // Listen for real-time measurements from WebSocket
  useEffect(() => {
    const unsubscribe = subscribe('NEW_MEASUREMENTS', (payload: any[]) => {
      // payload contains array of new measurements: [{ equipment_id, temperature, vibration, ... }]
      const timeStr = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
      
      setLiveData(prev => {
        const updated = { ...prev };
        payload.forEach(m => {
          const eqId = m.equipment_id;
          const currentPoints = updated[eqId] || [];
          
          const newPoint: ChartPoint = {
            time: timeStr,
            temperature: parseFloat(m.temperature),
            vibration: parseFloat(m.vibration),
            pressure: parseFloat(m.pressure),
            voltage: parseFloat(m.voltage),
            consumption: parseFloat(m.consumption)
          };
          
          // Keep last 20 points
          const newPoints = [...currentPoints, newPoint].slice(-20);
          updated[eqId] = newPoints;
        });
        return updated;
      });

      // Dynamically update dashboard stats
      setStats(prev => {
        const newConsumption = payload.reduce((sum, item) => sum + parseFloat(item.consumption), 0);
        return {
          ...prev,
          total_consumption: parseFloat(newConsumption.toFixed(2))
        };
      });
    });

    return () => unsubscribe();
  }, [subscribe]);

  // Listen for real-time alerts to increment the alert count
  useEffect(() => {
    const unsubscribe = subscribe('NEW_ALERT', () => {
      setStats(prev => ({ ...prev, active_alerts: prev.active_alerts + 1 }));
    });
    return () => unsubscribe();
  }, [subscribe]);

  const handleEquipmentChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    const id = parseInt(e.target.value, 10);
    setSelectedEquipId(id);
    
    if (!liveData[id] || liveData[id].length === 0) {
      try {
        const historyRes = await api.get(`/equipments/${id}/measurements?limit=15`);
        const formattedHistory: ChartPoint[] = historyRes.data.reverse().map((m: any) => ({
          time: new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
          temperature: parseFloat(m.temperature),
          vibration: parseFloat(m.vibration),
          pressure: parseFloat(m.pressure),
          voltage: parseFloat(m.voltage),
          consumption: parseFloat(m.consumption)
        }));
        setLiveData(prev => ({ ...prev, [id]: formattedHistory }));
      } catch (err) {
        console.error("Failed to fetch equipment measurements:", err);
      }
    }
  };

  const currentChartData = selectedEquipId ? (liveData[selectedEquipId] || []) : [];
  const selectedEquipment = equipments.find(e => e.id === selectedEquipId);

  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#0b0f19]">
        <div className="flex flex-col items-center gap-4">
          <Activity className="w-12 h-12 text-cyan-400 animate-spin" />
          <p className="text-slate-400 font-semibold">Chargement des données de supervision...</p>
        </div>
      </div>
    );
  }

  const handleExportReport = async () => {
    try {
      const response = await api.get('/reports/export', { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Rapport_Supervision_SmartMonitor.csv');
      document.body.appendChild(link);
      link.click();
      link.parentNode?.removeChild(link);
    } catch (error) {
      console.error("Erreur lors de l'exportation du rapport :", error);
      alert("Impossible de télécharger le rapport.");
    }
  };

  return (
    <div className="min-h-screen bg-[#0b0f19] pl-64 pr-6 py-8 flex flex-col gap-8">
      
      {/* Page Header */}
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-extrabold text-white tracking-tight">Tableau de Bord</h2>
          <p className="text-slate-400 text-sm mt-1">Supervision temps réel et analyse télémétrique globale</p>
        </div>
        <div className="flex items-center gap-4">
          <button
            onClick={handleExportReport}
            className="flex items-center gap-2 px-4 py-2.5 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 text-slate-300 hover:text-white text-sm font-semibold rounded-2xl transition-all duration-200"
          >
            <Download className="w-4 h-4 text-cyan-400" />
            <span>Exporter Rapport CSV</span>
          </button>
          <div className="flex items-center gap-3 bg-white/5 border border-white/5 px-4 py-2.5 rounded-2xl">
            <Clock className="w-5 h-5 text-cyan-400 animate-pulse-slow" />
            <span className="text-sm font-semibold text-white">Live Monitoring actif</span>
          </div>
        </div>
      </div>

      {/* KPI Cards Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        
        {/* Availability */}
        <div className="glass-card p-6 rounded-2xl flex items-center justify-between">
          <div className="flex flex-col gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Taux de Disponibilité</span>
            <span className="text-3xl font-extrabold text-white">{stats.availability_rate}%</span>
            <span className="text-[10px] text-emerald-400 font-bold flex items-center gap-1">
              <CheckCircle className="w-3 h-3" /> {stats.running_equipments} machines actives
            </span>
          </div>
          <div className="w-12 h-12 rounded-xl bg-cyan-500/10 flex items-center justify-center text-cyan-400 glow-cyan">
            <Compass className="w-6 h-6" />
          </div>
        </div>

        {/* Active Alerts */}
        <div className={`glass-card p-6 rounded-2xl flex items-center justify-between ${stats.active_alerts > 0 ? 'border-rose-500/20' : ''}`}>
          <div className="flex flex-col gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Alertes Actives</span>
            <span className={`text-3xl font-extrabold ${stats.active_alerts > 0 ? 'text-rose-400' : 'text-white'}`}>
              {stats.active_alerts}
            </span>
            <span className={`text-[10px] font-bold ${stats.active_alerts > 0 ? 'text-rose-400 animate-pulse' : 'text-slate-500'}`}>
              {stats.active_alerts > 0 ? '⚠️ Action requise' : 'Aucune anomalie'}
            </span>
          </div>
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${stats.active_alerts > 0 ? 'bg-rose-500/10 text-rose-400 glow-red' : 'bg-slate-800 text-slate-400'}`}>
            <AlertTriangle className="w-6 h-6" />
          </div>
        </div>

        {/* Energy Consumption */}
        <div className="glass-card p-6 rounded-2xl flex items-center justify-between">
          <div className="flex flex-col gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Consommation Totale</span>
            <span className="text-3xl font-extrabold text-white">{stats.total_consumption} kW</span>
            <span className="text-[10px] text-cyan-400 font-bold">Puissance active cumulée</span>
          </div>
          <div className="w-12 h-12 rounded-xl bg-amber-500/10 flex items-center justify-center text-amber-400">
            <Zap className="w-6 h-6" />
          </div>
        </div>

        {/* Equipments Count */}
        <div className="glass-card p-6 rounded-2xl flex items-center justify-between">
          <div className="flex flex-col gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Parc Machines</span>
            <span className="text-3xl font-extrabold text-white">{stats.total_equipments}</span>
            <span className="text-[10px] text-slate-400 font-bold">Équipements supervisés</span>
          </div>
          <div className="w-12 h-12 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-400">
            <Cpu className="w-6 h-6" />
          </div>
        </div>

      </div>

      {/* Main Charts & Controls Panel */}
      <div className="glass-panel p-6 rounded-3xl flex flex-col gap-6">
        
        {/* Controls Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-4">
          <div>
            <h3 className="text-lg font-bold text-white">Graphiques de Télémétrie en Direct</h3>
            <p className="text-xs text-slate-400">Visualisation en temps réel des variables physiques de l'équipement sélectionné</p>
          </div>
          <div className="flex items-center gap-3">
            <label htmlFor="equipment-select" className="text-xs font-semibold text-slate-400 uppercase tracking-wider">
              Machine :
            </label>
            <select
              id="equipment-select"
              value={selectedEquipId || ''}
              onChange={handleEquipmentChange}
              className="bg-slate-900 border border-white/10 text-white text-sm rounded-xl px-4 py-2 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500/20 font-medium"
            >
              {equipments.map(eq => (
                <option key={eq.id} value={eq.id}>{eq.name} ({eq.type})</option>
              ))}
            </select>
          </div>
        </div>

        {/* Selected Equipment Status */}
        {selectedEquipment && (
          <div className="flex gap-6 items-center bg-white/5 border border-white/5 p-4 rounded-2xl text-sm">
            <div>
              <span className="text-slate-400 font-medium">Localisation :</span>
              <span className="ml-2 font-bold text-white">{selectedEquipment.location}</span>
            </div>
            <div className="w-px h-4 bg-white/10" />
            <div>
              <span className="text-slate-400 font-medium">Statut :</span>
              <span className={`ml-2 px-2.5 py-0.5 text-xs font-bold rounded-full ${
                selectedEquipment.status === 'En fonctionnement' ? 'bg-emerald-500/10 text-emerald-400' :
                selectedEquipment.status === 'Maintenance' ? 'bg-amber-500/10 text-amber-400' :
                selectedEquipment.status === 'Anomalie' ? 'bg-rose-500/10 text-rose-400' :
                'bg-slate-800 text-slate-400'
              }`}>
                {selectedEquipment.status}
              </span>
            </div>
          </div>
        )}

        {/* Recharts Plots Grid */}
        {currentChartData.length === 0 ? (
          <div className="h-96 flex items-center justify-center border border-dashed border-white/10 rounded-2xl">
            <p className="text-slate-400 font-medium">En attente de réception des premières mesures capteurs...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            
            {/* Temperature & Vibration Chart */}
            <div className="glass-card p-4 rounded-2xl flex flex-col gap-4">
              <h4 className="text-sm font-bold text-white px-2">Température (°C) & Vibrations (mm/s)</h4>
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={currentChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                    <XAxis dataKey="time" stroke="#64748b" fontSize={10} />
                    <YAxis stroke="#64748b" fontSize={10} />
                    <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: 'rgba(255,255,255,0.1)', color: '#f8fafc' }} />
                    <Legend wrapperStyle={{ fontSize: 11 }} />
                    <Line type="monotone" dataKey="temperature" name="Température" stroke="#ef4444" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
                    <Line type="monotone" dataKey="vibration" name="Vibrations" stroke="#06b6d4" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>

            {/* Pressure & Consumption Chart */}
            <div className="glass-card p-4 rounded-2xl flex flex-col gap-4">
              <h4 className="text-sm font-bold text-white px-2">Pression (bar) & Consommation (kW)</h4>
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={currentChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                    <XAxis dataKey="time" stroke="#64748b" fontSize={10} />
                    <YAxis stroke="#64748b" fontSize={10} />
                    <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: 'rgba(255,255,255,0.1)', color: '#f8fafc' }} />
                    <Legend wrapperStyle={{ fontSize: 11 }} />
                    <Line type="monotone" dataKey="pressure" name="Pression" stroke="#eab308" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
                    <Line type="monotone" dataKey="consumption" name="Consommation" stroke="#10b981" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>

          </div>
        )}

      </div>
    </div>
  );
};

export default DashboardPage;
